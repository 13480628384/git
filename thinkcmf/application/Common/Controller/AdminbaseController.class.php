...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
		...
		$user_roles = M('sys_user_role')->where(array('user_id'=>get_current_admin_id()))->getField('role_id');
		$this->role_type = M('sys_role')->where(array('id'=>$user_roles))->getField('role_type');
		$this->assign('role_type',$this->role_type );
	    //暂时取消后台多语言
	    $this->load_app_admin_menu_lang();
	    //设备类型
		$this->device_type = M('device_type')->order('create_date asc')->select();
		$this->assign('device_type',$this->device_type);
	    $session_admin_id=session('ADMIN_ID');
	    
    	if(!empty($session_admin_id)){
    		$users_obj= M("sys_user");
    		$user=$users_obj->where(array('id'=>$session_admin_id,'del_flag'=>0))->find();
	 ...
	 * @param string $custom_field 自定义排序字段 默认为listorder,可以改为自己的排序字段
	 */
	protected function _percents($model,$custom_field='') {
		if (!is_object($model)) {
			return false;
		}
		$field=empty($custom_field)&&is_string($custom_field)?'percent':$custom_field;
		$pk = $model->getPk(); //获取主键名称
		$ids = $_POST['percent'];
		foreach ($ids as $key => $r) {
			$data[$field] = $r;
