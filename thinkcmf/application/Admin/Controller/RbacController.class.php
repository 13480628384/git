...
class RbacController extends AdminbaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->role_model = D("sys_role");
    }

    // 角色管理列表
    public function index() {
        $data = $this->role_model->order(array("create_date" => "ASC", "id" => "DESC"))->select();
    ...
    public function roleadd() {
		$office = M('sys_office')->where(array('del_flag'=>0,'type'=>array('neq','1')))->select();
    ...
    public function roleadd_post() {
    	if (IS_POST) {
			...
			$office_id = M('sys_user')->where(array('del_flag'=>0,'id'=>$user_id))->getField('office_id');
			$name = $_POST['name'];
