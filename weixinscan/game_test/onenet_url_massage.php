...
//初始化日志
$logHandler= new CLogFileHandler("logs/onenet/".date('Y-m-d').'-massage.log');
...
if(isPost()){
	...
	if($type == 2){
		...
		/*====找出某个设备昨天的数量===*/
		$YesterdayCountDevicesOne = $db->get_row("select * from device_record  where TO_DAYS(NOW()) - TO_DAYS(create_date) = 1 and dev_id='$dev_id'");
