<?php if (!defined('THINK_PATH')) exit(); if(is_array($friend_info)): $i = 0; $__LIST__ = $friend_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><div class="comm-total">
        ...
        <div class="com-middle" onclick="window.location.href='<?php echo U(detail,array(id=>$value[id],openid=>$value[create_by]));?>'">
            <?php echo ($value["text"]); ?>
        </div>
        <!--图片显示 [[-->
        ...
        <ul class="com-bottom">
            ...
            <li class="bottom-one" <?php if($is_openid != ''): ?>onclick="window.location.href='<?php echo U('com_reply',array('replybuyer_id'=>$value[create_by],'friendinfo_id'=>$value[id],'userid'=>$userid));?>'"<?php else: ?>onclick="window.location.href='<?php echo U('get_openid');?>'"<?php endif; ?>>
