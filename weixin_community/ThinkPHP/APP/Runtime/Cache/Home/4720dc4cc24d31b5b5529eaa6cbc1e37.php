...
<div class="space-60"></div>
    ...
    <ul>
        ...
        <li id="personal" onclick="window.location.href='<?php echo U('personal');?>'" class="current">
          个人信息
        </li>
    </ul>
  </menu>
...
//手动输入硬编码，自动验证
function hcsoft(){
        $.post("<?php echo U('device_code_exists_or');?>",{hard_device_code:$('#device_command').val()},function(data){
          if(data.msg == 1){
...
//手动输入软编码，自动验证
function pcsoft(){
        $.post("<?php echo U('query_group');?>",{device_code:$('#device_code').val()},function(data){
          if(data.msg == 1){
