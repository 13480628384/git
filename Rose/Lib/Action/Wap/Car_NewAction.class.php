<?php
class Car_NewAction extends BackAction{
    ...
    public function car_start_pay(){
        ...
        if($weixin_alipay_type == 'wechat'){
            ...
            $count = $total1-$total2;
            //第一次用户免费玩
           /* $one = M('car_member_pay')->where(array('openid'=>$openid))->find();
            $two = M('device_consume_rec')->where(array( 'from_username' => $openid))->find();
            $three = M('weixin_pay_rec')->where(array( 'from_username' => $openid))->find();
            if(empty($two)){
                $sms = array("TG_NUM"=>intval($price));
                $this->zizhu->send_data_to_edp($device_command, 1, 0, $sms);
            }*/
            //会员余额
            $car_member = M('car_member_pay')->where(array('openid'=>$openid,
                'status'=>'1'))->sum('account');
            if( $count<=0 && $car_member<0) {
            ...
            if($car_member>0){
                $car_member = M('car_member_pay')->where(array('openid'=>$openid,
                    'status'=>'1'))->order('create_date asc')->limit(1)->setDec('account',6);
            }else if($count>0){
            ...
            if($command_infos) {
                ...
                echo json_encode(array('code'=>200,'msg'=>'已经开始工作','count'=>intval($countall),'member'=>$car_member));
            } else {
                $model->rollback();
                echo json_encode(array('code'=>500,'msg'=>'临时维护中','member'=>$car_member));
            }
