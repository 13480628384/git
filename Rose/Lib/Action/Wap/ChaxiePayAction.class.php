 ...
 * 微信支付
 * */
class ChaxiePayAction extends BackAction{
    ...
    public function pay(){
        ...
        $jsapi = $this->Weixin_Pay_Result($openid,$price*100,
            get_dirname_url().'Wap/Notify/chaxienotify',$out_trade_no);
    ...
    public function start(){
        ...
        if($weixin_alipay_type == 'wechat'){
            ...
            $sms = array("T_M"=>intval($times),'P_Y'=>intval($price),'TG_MES'=>only_order());
            $model = M('device_consume_rec');
            $model->startTrans();
            $result = $this->chaxie->send_data_to_edp($device_command, $qos, $timeout, $sms);
            $return_result = 0;
            if (empty($result)) {
                $return_result = 0;
                $error_code = $this->chaxie->error_no();
                $error = $this->chaxie->error();
