<?php
//洗衣机
class WashingAction extends BackAction{
    ...
    protected function _initialize(){
        ...
        if( strpos($typesd,'MicroMessenger')>0 ){
            ...
            $this->type = 'wechat';
            /*$counts = M('weixin_userinfo')->where(array('status'=>1,
                'del_flag'=>0,
                'app_id'=>WxPayConfig::APPID,
                'from_username'=>trim($_GET['openid'])
            ...
            /*$counts = M('alipay_userinfo')->where(array(
                'status'=>1,
                'del_flag'=>0,
        ...
        $dgi = M('device_relation_group')->where(array('di_id'=>$device_info['id'],'del_flag'=>0,'device_type'=>5))->find();
