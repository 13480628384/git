...
<div class="addvideo" style="display: none;">
    <ul>
        <li><a href="<?php echo U('alipay_pay',array('buyer_id'=>$buyer_id,'device_code'=>$res['device_code']));?>">我的支付记录</a></li>
        <li style="border: none">
            <a href="<?php echo U('alipay_consume',array('buyer_id'=>$buyer_id,'device_code'=>$res['device_code']));?>">我的消费记录</a></li>
    </ul>
</div>
    ...
    <ul class="indent-tab-ul">
        <li>
            <?php if(is_array($shop)): $i = 0; $__LIST__ = $shop;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v['id'] != null): ?><div class="indent-tab-title">
                        ...
                        <span class="leftd">
                            <?php if($v['status'] == 0): ?><tt style="color:#f00">该通道已停用</tt>
                                <?php elseif($v['status'] == 1): ?><tt style="color:#59dc7d">有货</tt>
                            <?php elseif($v['status'] == 2): ?><tt style="color:#f00">该通道缺货</tt>
                            <?php elseif($v['status'] == 3): ?><tt style="color:#f00">该通道错误</tt>
                            <?php elseif($v['status'] == 4): ?><tt style="color:#f00">该通道正在忙</tt>
                                </else>其他问题<?php endif; ?> 余货：<?php echo ($v["shop_number"]); ?>
                        </span>
                            <span class="indent-btn buy_goods" num="<?php echo ($v["number"]); ?>" shopid="<?php echo ($v["id"]); ?>" dataprice="<?php echo ($v["shop_price"]); ?>" dataid="<?php echo ($v["gid"]); ?>" datastatus="<?php echo ($v["status"]); ?>" style="float:right" >
                            <a>
                                <span class="colorf57789">购买</span>
                            </a>
                        </span>
...
<div class="balance" style="background-color: #D33838; color: #FFF;width: 100%; height: 48px; text-align: center;line-height: 48px; position: fixed; bottom: 0px;  letter-spacing: 1px;font-size:14px">
    去充值&nbsp;<?php echo ($count); ?>元
</div>
<input type="hidden" id="balance_recharge" value="<?php echo U('VendingAlipay/balance_recharge');?>">
