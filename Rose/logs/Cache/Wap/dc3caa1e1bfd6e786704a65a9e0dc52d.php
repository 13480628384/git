...
<div class="facility" data-ng-app="myApp" ng-controller="myCtrl">
    ...
    <div class="tishi_yanz">价格1
        <input  ng-model="firstName1" type="text" placeholder="价格" pattern="^([0-9]\d*)$" required="required" style="width: 20%">元
        <input  ng-model="lastName1" type="text" placeholder="段" pattern="^([1-9]\d*)$" required="required" style="width: 20%">段</div>
    <div class="tishi_yanz">价格2
        <input  ng-model="firstName2" type="text" placeholder="价格" pattern="^([0-9]\d*)$" required="required" style="width: 20%">元
        <input  ng-model="lastName2" type="text" placeholder="时间" pattern="^([1-9]\d*)$" required="required" style="width: 20%">段</div>

    <div class="tishi_yanz">价格段<input id="srdata" type="text" value="{{firstName1 + '=' + lastName1+ '-' + firstName2 + '=' +lastName2}}"></div>
...
<script>
    ...
    Zepto(function($){
        $('.anniu').tap(function(){
            ...
            $.post("<?php echo U('update_ceji');?>",{openid:openid,srdata:srdata,di_id:di_id},function(data){
                if(data.code == 200){
                    var DG=$.dialog({
                        content:data.msg,
                        button:['好']
