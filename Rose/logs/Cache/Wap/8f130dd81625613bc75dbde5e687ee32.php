...
<div class="facility">
    ...
    <div class="tishi_yanz">群组名称<input type="text" class="group_name" value="<?php echo ($group_name); ?>"></div>
    <button class="anniu" id="edit_name">确定修改</button>
</div>
<input type="hidden" class="uid" value="<?php echo ($id); ?>">
...
<script>
    Zepto(function($){
        $('#edit_name').tap(function(){
            ...
            $.post("<?php echo U('update_device_group_info');?>",{device_group_name:group_name,uid:uid},function(reg){
                if(reg.msg==1){
                    var DG=$.dialog({
                        content:'恭喜您，修改成功！',
                        button:['好']
