...
class OneNetApi
{
    ...
    public function device_list( $page = 1, $is_online = NULL,$page_size = 100, $key_word = NULL, $tag = NULL, $is_private = NULL, $device_ids = NULL)
    {
        $params = array(
            'page' => is_numeric($page) ? $page : 1,
            'per_page' => is_numeric($page_size) ? $page_size : 100
        );
    ...
    public function device_lists_line( $is_online = NULL,$page = 1,$page_size = 100, $key_word = NULL, $tag = NULL, $is_private = NULL, $device_ids = NULL)
    {
        $params = array(
            'page' => is_numeric($page) ? $page : 1,
            'per_page' => is_numeric($page_size) ? $page_size : 100
    ...
    public function get_http($cmd_uuid){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,$this->_base_url.'/cmds/'.$cmd_uuid.'/resp');
        curl_setopt($ch, CURLOPT_TIMEOUT, 2);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
