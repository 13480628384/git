 ...
 * 机构绑定二维码
 */
function qrcode($secenid,$token,$openid)
{
    $id = substr($secenid,3);
    if($id && $token && $openid){
        $kdata = M('Organization')->where(array(
            'og_no' => $id,
            'token'   => $token
        ))->find();
        return $return_data  = array(
            array(
                ...
                '绑定机构名字:'.$kdata['name'],
                '',
                C('site_url') . '/index.php?g=Wap&m=Bind&a=index&token=' . $token . '&openid=' .$openid. '&og_id='.$kdata['id']
            )
        );
    }
}
