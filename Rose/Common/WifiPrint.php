...
class WifiPrint {
	function __construct($token){
		//xxxxxx
		include_once './Class/FeyinAPI/HttpClient.class.php';
		$feyinapi=M('feyinapi')->where(array('token'=>$token))->find();
		define('MEMBER_CODE',$feyinapi['name1']);
		define('FEYIN_KEY',$feyinapi['name2']);
		define('DEVICE_NO',$feyinapi['name3']);
	...
	function test($content){
		...
		*/
		...
		$freeMessage = array(
		 ...
		 return self::sendFreeMessage($freeMessage);
	...
	function testSendFormatedMessage(){
		...
		*/
		...
		self::sendFormatedMessage($msgInfo);
